...
namespace GremlinNetSample
{
    ...
    class Program
    {
        ...
        private static string Host => Environment.GetEnvironmentVariable("Host") ?? "demo-graphdb.gremlin.cosmos.azure.com";
        private static string PrimaryKey => Environment.GetEnvironmentVariable("PrimaryKey") ?? "DxEgrRMGHB2NN6Is0WutLhk0AkI6A9eSWD0zpWYmvONfY1HB7jdLldWRP9V71PmuQYcO3PRno1CfojZMBCCj9w==";
        private static string Database => Environment.GetEnvironmentVariable("DatabaseName") ?? "graphdb";
        private static string Container => Environment.GetEnvironmentVariable("ContainerName") ?? "Persons";
        ...
        private static Dictionary<string, string> gremlinQueries = new Dictionary<string, string>
        {
            ...
            { "AddVertex 5",    "g.addV('sports').property('id', 'cricket').property('partitionKey', 'sports')" },
            { "AddVertex 6",    "g.addV('sports').property('id', 'football').property('partitionKey', 'sports')" },


            { "AddEdge 5",      "g.V('thomas').addE('play').to(g.V('cricket'))" },
            { "AddEdge 6",      "g.V('robin').addE('play').to(g.V('cricket'))" },


            { "AddEdge 1",      "g.V('thomas').addE('knows').to(g.V('mary'))" },
            { "AddEdge 2",      "g.V('thomas').addE('knows').to(g.V('ben'))" },
            { "AddEdge 3",      "g.V('ben').addE('knows').to(g.V('robin'))" },
            { "AddEdge 4",      "g.V('thomas').addE('knows').to(g.V('robin'))" },
